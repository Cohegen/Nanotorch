...
The dataloader is the component which transforms individual dataset samples into batches that  neural networks crave.
...
```
We have built the **data loading infrastructure** that powers all modern ML:
- Dataset abstraction 
- TensorDataset - wraps one or more tensors into a single dataset object.
- Dataloader - responsible for batching, shuffling, iteration.
...
The key insight is shuffling overhead is typically negligible compared to the actual data loading and tensor operations.
