...
class LeNetDigitsOptimized:
    """
    A small LeNet-style CNN using OPTIMIZED (im2col) convolutions.
    """

    def __init__(self, num_classes=10):
        # Using optimized im2col method
        self.conv1 = Conv2d(in_channels=1, out_channels=6, kernel_size=3, padding=1, method='im2col')
        self.pool1 = MaxPool2d(kernel_size=2, stride=2)
        self.conv2 = Conv2d(in_channels=6, out_channels=16, kernel_size=3, padding=1, method='im2col')
...
def main(epochs=10, batch_size=32, learning_rate=0.03):
    (train_images, train_labels), (test_images, test_labels) = load_nanodigits()

    model = LeNetDigitsOptimized(num_classes=10)
    optimizer = SGD(model.parameters(), lr=learning_rate)
    loss_fn = CrossEntropyLoss()

    print("--- Training LeNetDigits (OPTIMIZED IM2COL) ---")
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1:02d} | Loss: {train_loss:.4f} | Acc: {train_acc:.4f} | Time: {duration:.2f}s")
    
    total_duration = time.time() - start_time
    print(f"Total Optimized Training Time: {total_duration:.2f}s")
